...
def infer_part_type(desc):
    ...
    if "seal" in d: return "Hydraulics"
...
def run_import():
    ...
    # Get existing part numbers to prevent duplicates
    try:
    ...
    for index, row in df.iterrows():
        try:
            ...
            # Skip if no description or part number seems invalid (NaN)
            if pd.isna(desc) or pd.isna(part_num):
                # Try Item # if Metso Part # is missing?
                # For now simplify: MUST have description.
                if pd.isna(desc):
                    continue
